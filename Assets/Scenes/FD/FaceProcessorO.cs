...
static partial class ArrayUtilities
        ...
        where T: UnityEngine.Texture
    {
        ...
        /// </summary>
        public DataStabilizerParamsO DataStabilizer { get; private set; }
        ...
        /// Detected objects
        /// </summary>
        public List<DetectedFaceO> Faces { get; private set; }

        /// <summary>
        /// Default constructor
        /// </summary>
        public FaceProcessorO()
        {
            Faces = new List<DetectedFaceO>();
            DataStabilizer = new DataStabilizerParamsO();
            ...
            if (detect)
            {
                ...
                for (int i = 0; i < rawFaces.Length; ++i)
                {
                    ...
                    using (Mat grayFace = new Mat(gray, faceRect))
                    {
                        ...
                        // get face object
                        DetectedFaceO face = null;
                        if (Faces.Count < i + 1)
                        {
                            face = new DetectedFaceO(DataStabilizer, faceRectScaled);
                            Faces.Add(face);
                        }
                        else
                        {
        ...
        {
            // mark each found eye
            foreach (DetectedFaceO face in Faces)
            {
                // face rect
                ...
                if (face.Info != null)
                {
                    foreach (DetectedFaceO.Triangle tr in face.Info.DelaunayTriangles)
    ...
    /// </summary>
    class FaceProcessorLive<T> : FaceProcessorO<T>
        where T : UnityEngine.Texture
    {
